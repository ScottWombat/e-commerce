...
import { BagMessage,BagEmptyMessage } from "app/components/bag/bag-message";
const ViewBag = () => {
    ...
    useEffect(() => {
        const msg = emptyBag?<BagEmptyMessage/>:<BagMessage/>
        ...
        if (data.length === 0) {
            ...
            setData(items)
            let grandTotal = (Math.round(g * 100) / 100) + flatRate;
            let f = grandTotal.toFixed(2)
            setGrandTotal(parseFloat(f))
            //setEmptyBag(true)
        }
        ...
        <div className={styles.container}>
            <div className={styles.left_container}>
                ...
                <div className={styles.header}>YOUR BAG</div>  
                  {emptyBag?<BagEmptyMessage/>:<BagMessage/>}
                </div>
            ...
            <div className={styles.right_container}>
                ...
                <div>&nbsp;</div>
                <div>
                    <button className={styles.checkout_button1} onClick={onCheckout}>GUEST CHECKOUT</button>
                    <div>&nbsp;</div>
                    <button className={styles.checkout_button}>MEMBER CHECKOUT</button>
                </div>
